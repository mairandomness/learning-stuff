...
/* each output line, followed by just some portions of the lines, to */
/* the standard output. */

/* The first input is a list of column numbers, which ends with a */
/* negative number. The column numbers are paired and specify */
/* ranges of columns from the the input line that are to be printed */
/* For example, 0 2 10 12 -1 indicates that only columns 0 through 3 */
...
int main (void)
{
  ...
  char input[MAX_INPUT];         // array or the input line
  char output[MAX_INPUT];        // array for the output line
...
int read_column_numbers( int columns[], int max)
{
  ...
  //make sure we have an even number of inputs, as they are supposed to be paires
  if (num % 2 != 0) {
...
void rearrange(char *output, char const *input, int n_columns, int const columns[])
{
  ...
  for (col =0; col < n_columns; col += 2){
    ...
    //if the input line isn't this long or the output array is full, we are done
