...
evenDivision ints
  -- numerator/divisor
 = do
  let numerators = take ((length ints) - 1) ints
  let division =
        [a `div` b | a <- numerators, b <- ints, a > b && ((a `mod` b) == 0)]
  head division

-- evenDivision :: [Int] -> Int
-- evenDivision ints = do
--   let twoByTwo = [(x, y) | x <- ints, y <- ints]
--   let noRepeats = filter (\(a, b) -> a /= b) twoByTwo
--   let even = filter (\(a, b) -> (a `mod` b) == 0) noRepeats
--   let elm = (head even)
--   (fst elm) `div` (snd elm)
main :: IO ()
main = do
  let inputLines = lines input
  let formattedInput = map words inputLines
  let numberLines = map (\line -> map toInt line) formattedInput
  let divs = map evenDivision numberLines
  let result = sum divs
